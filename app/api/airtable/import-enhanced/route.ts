          ...
          if (!dryRun) {
            ...
            const { error } = await supabase
              ...
              .upsert({
                ...
                company_unique_id: vehicle.companyUniqueId,
                airtable_vid_number: vehicle.airtableVidNumber,
                reminder_state: vehicle.reminderState,
                weekly_inspection_state: vehicle.weeklyInspectionState,
                last_inspection_date: vehicle.lastInspectionDate,
                last_maintenance_date: vehicle.lastMaintenanceDate,
                last_maintenance_mileage: vehicle.lastMaintenanceMileage,
                vehicle_state: vehicle.vehicleState,
                book_status: vehicle.bookStatus,
                alert_method: vehicle.alertMethod,
                vehicle_system: vehicle.vehicleSystem,
                new_vehicle_condition: vehicle.newVehicleCondition,
                starting_mileage: vehicle.startingMileage,
                acquisition_value: vehicle.acquisitionValue,
                placed_in_service: vehicle.placedInService,
                loan_maturity_date: vehicle.loanMaturityDate,
                depreciation_method: vehicle.depreciationMethod,
            ...
            if (role === 'mechanic') {
              ...
              const { data: user, error: userError } = await supabase
                ...
                .upsert({
                  ...
                  role: 'mechanic',
                  approval_status: 'approved',
                  member_legacy_id: member.memberLegacyId,
                  preferred_language: member.preferredLanguage,
                  notes: member.notes,
                  level_certification: member.levelCertification,
                  equipment_oversight: member.equipmentOversight,
          ...
          if (!dryRun) {
            const { error } = await supabase
              ...
              .upsert({
                ...
                legacy_service_id: request.legacyServiceId,
                legacy_autonumber: request.legacyAutonumber,
                jotform_id: request.jotformId,
