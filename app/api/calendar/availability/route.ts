...
export async function GET(request: NextRequest) {
  try {
    ...
    if (weekStart) {
      weekStartDate = new Date(weekStart + "T12:00:00");
    } else if (date) {
    ...
    if (date) {
      // Parse date at noon to avoid timezone day shift
      const checkDate = new Date(date + "T12:00:00");
      const dayOfWeek = checkDate.getDay();
      ...
      const dateForCompare = new Date(date + "T00:00:00");
      ...
      if (workingDays.includes(dayOfWeek) && bookingsRemaining > 0 && isWithinAdvanceWindow) {
        // Get all bookings for this date in ONE query (not inside the loop)
