...
export function useCreateVehicle() {
  ...
  return useMutation({
    ...
    onMutate: async (newVehicle) => {
      ...
      queryClient.setQueryData(queryKeys.vehicles, (old: Vehicle[] = []) => [optimisticVehicle, ...old]);
    ...
    onSuccess: (data) => {
      ...
      queryClient.setQueryData(queryKeys.vehicles, (old: Vehicle[] = []) =>
        old.map((vehicle) => (vehicle.id.startsWith("temp-") ? data.vehicle : vehicle))
      );
    },
    onSettled: () => {
...
export function useUpdateVehicle() {
  ...
  return useMutation({
    ...
    onMutate: async ({ id, updates }) => {
      ...
      // Optimistic update
      queryClient.setQueryData(queryKeys.vehicles, (old: Vehicle[] = []) =>
        old.map((vehicle) => (vehicle.id === id ? { ...vehicle, ...updates } : vehicle))
      );
    ...
    onSuccess: (data) => {
      queryClient.setQueryData(queryKeys.vehicles, (old: Vehicle[] = []) =>
        old.map((vehicle) => (vehicle.id === data.vehicle.id ? data.vehicle : vehicle))
      );
...
export function useDeleteVehicle() {
  ...
  return useMutation({
    ...
    onMutate: async (id) => {
      ...
      queryClient.setQueryData(queryKeys.vehicles, (old: Vehicle[] = []) => old.filter((vehicle) => vehicle.id !== id));
...
export function useCreateDriver() {
  ...
  return useMutation({
    ...
    onMutate: async (newDriver) => {
      ...
      queryClient.setQueryData(queryKeys.drivers, (old: User[] = []) => [optimisticDriver, ...old]);
    ...
    onSuccess: (data) => {
      queryClient.setQueryData(queryKeys.drivers, (old: User[] = []) => old.map((driver) => (driver.id.startsWith("temp-") ? data.driver : driver)));
    },
    onSettled: () => {
