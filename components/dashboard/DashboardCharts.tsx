...
export default function DashboardCharts({ stats }: DashboardChartsProps) {
  ...
  const bookingCompletionRate =
    bookingTotals.total > 0 ? Math.round((bookingTotals.completed / bookingTotals.total) * 100) : 0
  const avgLeadTime = Math.round((stats.avgBookingLeadTimeDays || 0) * 10) / 10
  const avgMaintenancePerJob =
    stats.completedJobs > 0
      ? Math.round((stats.totalMaintenanceCost / stats.completedJobs) * 10) / 10
      : 0
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="card p-6">
                  ...
                  <BarChart data={repairUrgencyData} margin={{ bottom: 6 }}>
                    ...
                    <Bar dataKey="count" radius={[8, 8, 0, 0]} barSize={22}>
                      {repairUrgencyData.map((entry, idx) => (
                        <Cell
                          key={entry.raw}
                          fill={URGENCY_COLOR[entry.raw] || COLORS[idx % COLORS.length]}
                        />
                      ))}
                    </Bar>
                  </BarChart>
            ...
            <p className="text-xs text-muted mt-3">
              {outstandingRepairs} requests awaiting scheduling/assignment
            </p>
          </div>
