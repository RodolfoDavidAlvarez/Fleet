...
interface DriverColumnConfigModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export default function DriverColumnConfigModal({ isOpen, onClose, columns, onColumnsChange }: DriverColumnConfigModalProps) {
              ...
              <div className="space-y-2">
                {sortedColumns.map((column, index) => (
                  ...
                  >
                    ...
                    <button onClick={() => handleToggleVisibility(column.id)} className="flex items-center gap-2 flex-1 text-left">
                      {column.visible ? <Eye className="h-4 w-4 text-green-600" /> : <EyeOff className="h-4 w-4 text-gray-400" />}
                      <span className={`text-sm font-medium ${column.visible ? "text-gray-900" : "text-gray-500"}`}>{column.label}</span>
                    </button>
