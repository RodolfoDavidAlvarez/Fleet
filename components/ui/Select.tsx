...
export default function Select({ value, onChange, options, placeholder = "Select an option", className = "", disabled = false }: SelectProps) {
  ...
  return (
    <div ref={selectRef} className={`relative ${className}`}>
      <button
        ...
        `}
      >
        <span className={selectedOption ? "text-gray-900 font-medium" : "text-gray-500"}>{selectedOption ? selectedOption.label : placeholder}</span>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute z-[100] w-full mt-2 bg-white border-2 border-gray-200 rounded-lg shadow-xl max-h-60 overflow-y-auto"
