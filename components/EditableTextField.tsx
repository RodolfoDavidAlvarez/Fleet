      ...
      <div className={`flex items-center gap-1 ${className}`} onClick={(e) => e.stopPropagation()}>
        <div className="relative">
          <input
            ...
            onBlur={handleBlur}
            className="px-2.5 py-1.5 text-sm border-2 border-primary-500 rounded-md focus:outline-none focus:ring-2 focus:ring-primary-200 bg-white min-w-[100px] shadow-sm"
            disabled={isSaving}
  ...
  return (
    ...
    >
      <span className="inline-flex items-center gap-1.5">
        {displayValue}
        {isHovered && (
          <Pencil className="h-3 w-3 text-primary-500 opacity-60 transition-opacity" />
        )}
      </span>
    </span>
  );
